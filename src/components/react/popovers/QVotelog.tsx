...
const VotelogContent = ({
  ...
  id,
  close,
}: Omit<QVotelogPopoverProps, "right"> & { close?: boolean }) => (
  <>
    ...
    <div className="px-15 py-10">
      ...
      <div className="mb-10 flex items-center justify-between font-kondolar">
        <p className="wv-h9 mr-auto font-bold">
          {Math.round(
            (((vote?.mp?.[0] ?? 0) + (vote?.senate?.[0] ?? 0)) /
              ((vote?.mp?.[5] ?? 0) + (vote?.senate?.[5] ?? 0) || 1)) *
              1e4
          ) / 1e2}
          % เห็นด้วย
        </p>
...
export const QVotelogContent = qwikify$(
  ({ pass, date, type, title, vote, id }: Omit<QVotelogPopoverProps, "right">) => (
...
export const QVotelog = qwikify$<QVotelogPopoverProps>(
  ({ pass, date, type, title, vote, id, right }) => (
    <Popover className="relative flex">
      ...
      <Transition
        ...
        entered={
          "lg:translate-y-[76.67px] " + (right ? "lg:right-0 lg:left-auto" : "lg:left-0")
        }
        leave={
          right
            ? "lg:right-0 lg:left-auto lg:origin-top-right"
