...
export const QPeople = qwikify$<QPeoplePopoverProps>(
  ({ data, imgBase }) => {
    const [isOpenedOnce, setIsOpenOnce] = useState(false);

    return imgBase === "" ? (
      <div className="h-[22px] w-[22px] rounded-full border bg-vote-งด" />
    ) : (
      <Popover className="relative flex">
        {({ open }) => (
          !isOpenedOnce && setIsOpenOnce(() => true),
          (
            <>
              <Popover.Button
              ...
              >
                <Popover.Panel
                  static
                  className="w-[300px] overflow-hidden rounded-10 border bg-white lg:w-[320px]"
                >
                  <div
                    className={`h-15 ${
                      data.SenatorMethod === "เลือกกันเอง"
